...
def resolve_source_key(args: argparse.Namespace) -> tuple[str, str, str]:
    if args.source_key:
        ...
        if "/" not in source_key:
            raise RuntimeError("source-key must include a collection prefix, for example evaluation/gertler_ch8.pdf")
...
def main() -> int:
    ...
    export_dir = Path(args.output_dir) if args.output_dir else export_root / collection_name / slugify(Path(relative_source).stem)
    ...
    for name, payload in files.items():
        (export_dir / name).write_text(json.dumps(payload, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
