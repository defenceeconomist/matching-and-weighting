...
dag {
  ...
  Prior_need -> Treatment
  Prior_need -> Outcome
  Treatment -> Outcome
}
")

ggdag::ggdag(dag_good_control, text = FALSE) +
...
dag {
  Motivation [pos=\"0,2\"]
  Treatment [pos=\"1,2\"]
  Application_complete [pos=\"1,1\"]
  Administrative_capacity [pos=\"2,2\"]
  Outcome [pos=\"3,2\"]

  Motivation -> Treatment
  Motivation -> Application_complete
  Administrative_capacity -> Application_complete
  Administrative_capacity -> Outcome
  Treatment -> Outcome
}
")

ggdag::ggdag(dag_collider_case, text = FALSE) +
  ggdag::geom_dag_point(
    shape = 21,
    fill = "grey92",
    color = NA,
...
dag {
  Prior_need [pos=\"0,2\"]
  Treatment [pos=\"1,2\"]
  Skill_gain_after_programme [pos=\"2,1\"]
  Outcome [pos=\"3,2\"]

  Prior_need -> Treatment
  Prior_need -> Outcome
  Treatment -> Skill_gain_after_programme
  Skill_gain_after_programme -> Outcome
  Treatment -> Outcome
}
")

ggdag::ggdag(dag_post_treatment, text = FALSE) +
  ggdag::geom_dag_point(
    shape = 21,
    fill = "grey92",
    color = NA,
