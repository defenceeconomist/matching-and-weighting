...
format:
  html:
    ...
    code-links:
      - text: Full Code
        href: matching_methods_report_full_code.html
      - text: QA Log
        href: matching_methods_report_qa_log.html
  pdf:
    toc: true
    number-sections: true
execute:
...
- This report compares exact matching, coarsened exact matching (CEM), and entropy balancing as design-stage tools for estimating an `ATT` from observational data.
- In the `lalonde` worked example, all three adjusted designs produce a positive programme-effect estimate, but they do so with different tradeoffs in transparency, covariate balance, and retained information.
...
For a benchmarked observational design closer to the NSW discussion in *The Mixtape*, see the [NSW + CPS Benchmark Lab](../labs/nsw_cps_benchmark_lab.qmd).
:::
...
For a compact visual version of that rule, see the [DAG and Bad Controls Appendix](dag_bad_controls_appendix.qmd).
...
For readers who do need that extension, the shortest bridge from this report is the [Propensity-Score Weighting Companion](propensity_score_weighting_companion.qmd). For a benchmark application that maps more directly onto *The Mixtape*'s NSW discussion, the next repo stop is the [NSW + CPS Benchmark Lab](../labs/nsw_cps_benchmark_lab.qmd).
...
All three adjusted designs yield positive point estimates for the `ATT`, which is reassuring at the level of direction. Exact matching produces the smallest estimated gain at `r treatment_effect_summary$estimate[treatment_effect_summary$method == "Exact matching"]`, the custom CEM design is somewhat larger at `r treatment_effect_summary$estimate[treatment_effect_summary$method == "Coarsened exact matching"]`, and entropy balancing produces the largest at `r treatment_effect_summary$estimate[treatment_effect_summary$method == "Entropy balancing"]`.

But the table also shows why the treatment-effect comparison cannot be separated from the design diagnostics. The CEM estimate is attached to a much narrower treated population because only `r treatment_effect_summary$treated_units_used[treatment_effect_summary$method == "Coarsened exact matching"]` treated units remain after pruning. Exact matching retains far more treated units, but it does so with a weaker design on the full covariate set because imbalance remains on `re74`. Entropy balancing preserves all treated units, yet its comparison rests on a weighted control sample with effective sample size `r round(ebal_summary$effective.sample.size["Weighted", "Control"], 3)`, so the estimate depends on a concentrated subset of controls rather than on the full raw control pool in any substantive sense.
