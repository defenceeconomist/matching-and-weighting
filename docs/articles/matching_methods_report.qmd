...
For a benchmarked observational design closer to the NSW discussion in *The Mixtape*, see the [NSW + Current Population Survey (CPS) Benchmark Lab](../labs/nsw_cps_benchmark_lab.qmd).
:::
...
cem_balance$Balance |>
  ...
  knitr::kable(caption = "Final CEM balance diagnostics from cobalt.")
```
...
## Appendix A: DAG and Bad Controls Companion {.appendix .unnumbered} 
...
```{r fig.cap="Good control DAG: pre-treatment confounder blocks backdoor path when adjusted.", fig.width=7, fig.height=4}
dag_good_control <- ggdag::dagify(
  ...
  coords = list(
    x = c(prior_need = 0, treatment = 1, outcome = 2),
    y = c(prior_need = 1, treatment = 1, outcome = 1)
  )
)

ggdag::ggdag(dag_good_control, text = FALSE, node = FALSE) +
  ...
  ggdag::geom_dag_text_repel(
    ...
    color = "#111111",
    bg.color = "#FFF7D6",
    point.padding = grid::unit(0.9, "lines"),
    box.padding = 0.2,
    seed = 123
  ) +
  ggdag::theme_dag() +
...
```{r fig.cap="Collider case DAG: conditioning on application completion opens a non-causal path.", fig.width=7.5, fig.height=4.5}
dag_collider_case <- ggdag::dagify(
  treatment ~ motivation,
  application_complete ~ motivation + administrative_capacity,
  outcome ~ treatment + administrative_capacity,
  exposure = "treatment",
  outcome = "outcome",
  labels = c(
...
ggdag::ggdag(dag_collider_case, text = FALSE, node = FALSE) +
  ggdag::geom_dag_point(
    shape = 16,
    color = "#6FA8DC",
    size = 14
...
```{r fig.cap="Post-treatment mediator DAG: conditioning on mediator changes estimand away from total effect.", fig.width=8, fig.height=4.5}
dag_post_treatment <- ggdag::dagify(
  treatment ~ prior_need,
  skill_gain_after_programme ~ treatment,
  outcome ~ treatment + prior_need + skill_gain_after_programme,
  exposure = "treatment",
  outcome = "outcome",
  labels = c(
    prior_need = "Prior need",
    treatment = "Treatment",
    skill_gain_after_programme = "Skill gain after programme",
    outcome = "Outcome"
  ),
  coords = list(
    x = c(
      prior_need = 0,
      treatment = 1,
      skill_gain_after_programme = 2,
      outcome = 3
    ),
    y = c(
      prior_need = 2,
      treatment = 2,
      skill_gain_after_programme = 1,
      outcome = 2
    )
  )
)

ggdag::ggdag(dag_post_treatment, text = FALSE, node = FALSE) +
  ggdag::geom_dag_point(
    shape = 16,
    color = "#6FA8DC",
    size = 14
...
- Use the [NSW + Current Population Survey (CPS) Benchmark Lab](../labs/nsw_cps_benchmark_lab.qmd) for a benchmarked observational case that applies the same design-first logic in a fuller observational setting.
...
## Appendix B: Propensity-Score Weighting Companion {.appendix .unnumbered}
...
4. If you want a benchmarked observational case, continue to the [NSW + Current Population Survey (CPS) Benchmark Lab](../labs/nsw_cps_benchmark_lab.qmd).
